...
xSemaphoreHandle xSemaphoreCounting = NULL;
...
void EXTI15_10_IRQHandler(void)
{
	...
	sprintf(mymesage,"Interupt is Actiove \r\n");
	printf(mymesage);
...
void Handler_Task(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  sprintf(mymesage,"Handler Task: Processing evnt \r\n");
	  printf(mymesage);
  }
  /* USER CODE END 5 */
}
...
void Periodic_Task(void *argument)
{
  ...
  for(;;)
  {
   ...
   sprintf(mymesage,"Periodic Task : Pending the Interup. \r\n");

   HAL_NVIC_SetPendingIRQ(EXTI15_10_IRQn);

   sprintf(mymesage,"Periodic Task : Resuming \r\n");  }
